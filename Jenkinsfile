            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'ssh-12', keyFileVariable: 'SSH_KEY_FILE', usernameVariable: 'SSH_USER')]) {
                    sh '''
                        ...
                        scp -i "$SSH_KEYF_ILE" webpage.html root@13.126.52.93:/usr/share/nginx/html/index.html
                        ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no $SSH_USER@13.126.52.93 "sudo systemctl restart nginx"
                    '''
                }
            }
        }
